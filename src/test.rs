...
impl TestCase {
    ...
    fn id(&self) -> TestCaseId {
        ...
        TestCaseId::new(&self.commit_hash, &self.test.name, &self.notes_object_hash)
    }
}
...
impl GraphNode for TestCase {
    ...
    fn child_ids(&self) -> Vec<impl Borrow<TestCaseId>> {
        self.test
            .depends_on
            .iter()
            .map(|test_name| TestCaseId::new(&self.commit_hash, test_name, &self.notes_object_hash))
            .collect()
    }
}
